...
void mat_mul(float *result, float *mat1, float *mat2, int rows1, int cols1, int cols2)
{
    ...
    int jndex = threadIdx.x;

    float part_sum = 0;

    for (int k = 0; k<cols1; k++)
    {
        part_sum += mat1[index*cols1 + k]*mat2[k*cols2 + jndex];
    }

    result[index*cols2 + jndex] = part_sum;
...
int main()
{
    ...
    // Launch kernel
    mat_mul<<<numBlocks, numThreads>>>(d_res, d_m1, d_m2, a, b, c);
